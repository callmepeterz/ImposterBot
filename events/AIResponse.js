...
module.exports = {
    ...
    async execute(message, attempt = 0){
        attempt ++;
        try {
            if(message.flags.has(MessageFlags.SuppressNotifications) || !message.content || !(message.content?.includes(`<@${message.client.user.id}>`) || message.content?.includes(`<@!${message.client.user.id}>`))) return;
